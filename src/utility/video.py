...
class VideoUtils:
    ...
    def load_to_tensor_using_cv2(vpath, oshape, data_aug_flag = False):
        ...
        props = VideoUtils.get_video_properties(vpath)
        ...
        while vo.isOpened():
            ...
            if ret:
                ...
                if data_aug_flag:
                    if aug_prob > 0.5:
                        frame = VideoUtils.apply_horizontal_flip(frame, hflip_prob)
                        frame = VideoUtils.apply_scaling(frame, rescaling_ratio)
                        frame = VideoUtils.apply_shearing(frame, shear_factor)
                        frame = VideoUtils.apply_rotation(frame, rot_angle)
                        frame = VideoUtils.apply_horizontal_translation(frame, w_translation)
